...
import me.vension.smallestwidth.demo.DimenTypes;
...
public class MakeUtils {
    ...
    public static void makeAll(int designWidth, DimenTypes type, String buildDir) {
        try {
            ...
            System.out.print("生成目标目录==>"+ file.getAbsolutePath());
            if (!file.exists()) {
                file.mkdirs();
            }
     ...
     * @param buildDir 生成的目标文件夹
     */
    public static void makeAll(int designWidth, int type, String buildDir) {
        try {
            //生成规则
            final String folderName;
            if (type > 0) {
                //适配Android 3.2+
                folderName = "values-sw" + String.valueOf(type) + "dp";
            }else {
                return;
            }
     ...
     * @return
     */
    private static String makeAllDimens(int type, int designWidth) {
        float dpValue;
        String temp;
        StringBuilder sb = new StringBuilder();
        try {
            ...
            sb.append(XML_RESOURCE_START);
            //备份生成的相关信息
            temp = String.format(XML_BASE_DPI, type);
            sb.append(temp);
            for (int i = 0; i <= MAX_SIZE; i++) {

                dpValue = px2dip((float) i,type,designWidth);
                temp = String.format(XML_DIMEN_TEMPLETE,"", i, dpValue);
                sb.append(temp);
            }
